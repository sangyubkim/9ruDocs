...
type Block =
  ...
  | { type: "p"; text: string }
  | { type: "li"; text: string }
  | { type: "img"; alt: string; uri: string }
  | { type: "spacer" };
...
function parseImage(line: string): { alt: string; uri: string } | null {
...
export function parseMarkdownBody(body: string): Block[] {
  ...
  for (const raw of lines) {
    ...
    if (img) {
      blocks.push({ type: "img", alt: img.alt, uri: img.uri });
      continue;
    }
    ...
    } else if (/^[✔✓]\s*(?:맛|가격|서비스|청결|재방문의사)\s*[★☆]+/.test(line.trim())) {
      // 별점 줄: 불릿 없이 세로로
            ...
            return (
              <View key={i} style={styles.imgWrap}>
                <Image
                  ...
                  style={styles.img}
                  accessibilityLabel={block.alt || "이미지"}
                />
              </View>
            );
          case "li":
...
const styles = StyleSheet.create({
  ...
  img: { width: "100%", height: 200, backgroundColor: "#f1f5f9" },
