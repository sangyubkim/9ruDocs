...
REM 루트에 잘못 설치된 expo 제거 (package.json 에 없어도 node_modules 만 남는 경우)
if exist "%ROOT_DIR%\node_modules\expo" (
  echo [0/5] 루트 node_modules 정리 (expo SDK 55 잔여물 — npx expo 오류 원인) ...
  rmdir /s /q "%ROOT_DIR%\node_modules" 2>nul
) else if exist "%ROOT_DIR%\node_modules" (
  echo [0/5] 루트 node_modules 정리 ...
  rmdir /s /q "%ROOT_DIR%\node_modules" 2>nul
)
if exist "%ROOT_DIR%\package-lock.json" (
  echo       루트 package-lock.json 삭제 ...
  del /f /q "%ROOT_DIR%\package-lock.json" 2>nul
)
